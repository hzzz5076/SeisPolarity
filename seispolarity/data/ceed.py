...
from .base import WaveformBenchmarkDataset, WaveformDataWriter, logger
...
class CEED(WaveformBenchmarkDataset):
    ...
    def _download_dataset(self, writer: WaveformDataWriter, chunk: str, **kwargs):
        ...
        logger.warning(f"Start chunk {chunk} from Huggingface Hub")
    ...
    def _adjust_hdf5(path: Path) -> None:
        with h5py.File(path, "a") as f:
            # SeisPolarity needs a data format group
            g = f.create_group("data_format")
            g.create_dataset("dimension_order", data="CW")
            # Add a softlink from "data/" to "/" as SeisPolarity expects all waveforms under "data/"
